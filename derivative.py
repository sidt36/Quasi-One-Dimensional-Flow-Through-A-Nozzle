...
def compute_diff_forward(density,velocity,gamma,temperature,delta_x,a,a1,i):
    ...
    def compute_diff_rear(density,velocity,gamma,temperature,delta_x,a,a1,i):
        i = i-1
        d1 = -density[i+1]*(velocity[i+1]-velocity[i])/(delta_x) - density[i]*velocity[i]*(np.log(a1)-np.log(a))/delta_x - velocity[i]*(density[i+1]-density[i])/delta_x
        d2 = -velocity[i]*(velocity[i+1]-velocity[i])/delta_x - (1/gamma)*((temperature[i+1]-temperature[i]))/delta_x + (temperature[i]/density[i])*(density[i+1]-density[i])/delta_x
        d3 = -velocity[i]*(temperature[i+1]-temperature[i])/delta_x - (gamma-1)*((temperature[i])/delta_x)*(velocity[i+1] - velocity[i] + velocity[i]*(np.log(a1)-np.log(a)))
        return d1,d2,d3
...
def diff_av_mccormack(Nx,density,temperature,velocity,delta_x,delta_t,gamma = 1.4):
    ...
    for i in range(Nx-2):
        a = area((i+1)*delta_x)
        a1 = area((i+2)*delta_x)
        d1,d2,d3 = compute_diff_forward((density),(velocity),gamma,(temperature),delta_x,a,a1,i+1)
        d_density[i+1] = d1
        d_velocity[i+1] = d2
    ...
    for i in range(Nx-2):
        a = area((i)*delta_x)
        a1 = area((i+1)*delta_x)
        d1,d2,d3 = compute_diff_rear(density_p,(velocity_p),gamma,(temperature_p),delta_x,a,a1,i+1)
